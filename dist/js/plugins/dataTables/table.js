...
function dataTablesInit(elo) {
  ...
  eloancn = $('#' + elo.tableId).dataTable({
    ...
    columns: elo.columns,
    searching: elo.gridInit.searching && true, //搜索框，默认是开启
    paging: elo.gridInit.paging && true, //是否开启本地分页，默认是开启
    scrollX: '100%', //设置宽度
    autoWidth: true, //是否自适应宽度
    scrollY: elo.gridInit.scrollY, //设置高
    scrollCollapse: elo.gridInit.scrollCollapse || false, //是否开启DataTables的高度自适应，当数据条数不够分页数据条数的时候，插件高度是否随数据条数而改变
